    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isNewsletter) {
                ...
                const newsletterMediaType = getMediaType(patched);
            ...
            const normalizedForNodes = normalizeMessageContent(message);
            if (isGroup && regexGroupOld.test(jid) && !message.reactionMessage) {
                stanza.content.push({ tag: 'multicast', attrs: {} });
            }
            const pollMessageForNodes = normalizedForNodes?.pollCreationMessage ||
                normalizedForNodes?.pollCreationMessageV2 ||
                normalizedForNodes?.pollCreationMessageV3;
            if (pollMessageForNodes || normalizedForNodes?.eventMessage) {
                stanza.content.push({
                    tag: 'meta',
                    attrs: normalizedForNodes?.eventMessage
                        ? { event_type: 'creation' }
                        : isNewsletter
                            ? {
                                polltype: 'creation',
                                contenttype: pollMessageForNodes?.pollContentType === 2 ? 'image' : 'text'
                            }
                            : { polltype: 'creation' }
                });
            }
            const buttonTypeForNodes = normalizedForNodes ? getButtonType(normalizedForNodes) : undefined;
            if (!isNewsletter && buttonTypeForNodes) {
                stanza.content.push(getButtonArgs(normalizedForNodes));
            }
