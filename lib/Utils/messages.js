...
// jimp v0.x exports a default class; jimp v1.x exports a named `Jimp` class.
// Resolving via the namespace import above works for both without a static
// default/named import that would throw on the "wrong" version.
const Jimp = JimpNS.Jimp || JimpNS.default || JimpNS;
/**
 * Resizes/compresses an image (URL, path, or Buffer) into a small JPEG thumbnail buffer,
 * used for location & button-location previews.
 */
async function processLocationThumbnail(input, options = {}) {
    ...
    else if (typeof image.contain === 'function') {
    ...
    return typeof image.getBufferAsync === 'function'
        ? await image.getBufferAsync(mime)
...
/**
 * Sends an album (multiple images/videos grouped together) as a sequence of messages
 * linked via messageContextInfo.messageAssociation, preceded by an albumMessage announcing
 * the expected image/video counts.
 * @param jid the chat to send the album to
 * @param albums array of media objects, each containing an `image` or `video` key
 * @param options prepare/generate options; must include `conn` (the socket used to relay the album message)
 */
...
export const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, 'text')) {
        ...
        if (!thumbInput)
            throw new Error('jpegThumbnail or thumbnail is required');
        ...
        if (!gs.message)
            throw new Error('groupStatus.message is required');
        ...
        if (typeof messageContent === 'string') {
            ...
            if (backgroundColor)
                messageContent.extendedTextMessage.backgroundArgb = backgroundColor;
            if (textColor)
                messageContent.extendedTextMessage.textArgb = textColor;
            if (font)
                messageContent.extendedTextMessage.font = font;
        ...
        if (!os.image)
            throw new Error('image is required for orderStatus');
        ...
        if (rich.text) {
            ...
            const inline_entities = parsed.ie ? parsed.ie.map(({ type, ie }) => {
                ...
                if (type === 'latex')
                    return { key: ie.key, metadata: { latex_expression: ie.text || '', latex_image: { url: ie.url, width: Number(ie.width) || 100, height: Number(ie.height) || 100 }, font_height: Number(ie.font_height) || 83.33, padding: Number(ie.padding) || 15, __typename: 'GenAILatexItem' } };
                return null;
            }).filter(Boolean) : [];
...
export const generateWAMessageFromContent = (jid, message, options) => {
    ...
    if (quoted && !isJidNewsletter(jid)) {
        ...
        if (key === 'requestPaymentMessage') {
            if (innerMessage?.requestPaymentMessage &&
            ...
            else if (innerMessage?.requestPaymentMessage &&
        ...
        else if (contextInfo && innerMessage[key]) {
            /* @ts-ignore */
...
export const downloadMediaMessage = async (message, type, options, ctx) => {
    ...
    async function downloadMsg() {
        ...
        const media = contentType === 'productMessage'
